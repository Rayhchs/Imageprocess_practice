...
void smooth_filter(func_in in) {

	// Smooth filter without OpenCV
...
int main(){
	...
	// W OpenCV
	cv::blur(img, filtered_img, cv::Size(3, 3));
    ...
    for (int i = 0; i < 3; ++i) {
    	...
    	in.k_size = 3;
    	in.img = channels[i];
        threads.emplace_back(smooth_filter, in);
    }
